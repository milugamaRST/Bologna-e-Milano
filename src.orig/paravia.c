...
// Side Effects:N/A
//
//This code is copyrighted and has// limited warranties.Please see http://www.Planet-Source-Code.com/vb/scripts/ShowCode.asp?txtCodeId=7183&lngWId;=3//for details.//**************************************
...
char CityList[7][15] = { "Santa Paravia", "Fiumaccio", "Torricella",
...
Me->Rats = Random(50);
Me->GrainReserve = ((Me->GrainReserve * 100) - (Me->GrainReserve * Me->Rats))
		/ 100;
return;
}
...
revenues = Me->CustomsDutyRevenue + Me->SalesTaxRevenue + Me->IncomeTaxRevenue
		+ Me->JusticeRevenue;
...
printf("%d\t\t%d\t\t%d\t\t%d %s\n", Me->CustomsDutyRevenue, Me->SalesTaxRevenue,
		Me->IncomeTaxRevenue, Me->JusticeRevenue, string);
return;
}
...
while (IsOK == False) {
	...
	printf("1 = Minimum (%d), 2 = Maximum(%d), or enter a value: ", Minimum,
			Maximum);
	fgets(string, 255, stdin);
	HowMuch = (int) atoi(string);
	if (HowMuch == 1)
...
ord = MyScale - (float) absc;
Me->NewSerfs =
		(int) ((((float) Random(absc) + ord) * (float) Me->Serfs) / 100.0);
...
while (Finished == False) {
	...
	printf("%d\t%d\t%d\t\t%.2f\t\t%d\n", Me->GrainReserve, Me->GrainDemand,
			Me->GrainPrice, Me->LandPrice, Me->Treasury);
...
while (val != 0 || string[0] != 'q') {
	printf("\n%s %s\n\n", Me->Title, Me->Name);
	GenerateIncome(Me);
	printf("(%d%%)\t\t(%d%%)\t\t(%d%%)", Me->CustomsDuty, Me->SalesTax,
			Me->IncomeTax);
...
for (; i < HowMany; i++)
	printf("\n%s %s\n%d\t%d\t\t%d\t%d\t\t%d\t%d\t%d\n", MyPlayers[i].Title,
			MyPlayers[i].Name, MyPlayers[i].Nobles, MyPlayers[i].Soldiers,
			MyPlayers[i].Clergy, MyPlayers[i].Merchants, MyPlayers[i].Serfs,
